...
public final class Dominators {
    ...
    private static int intersect(int[] idom,int[] rpoPos,int a0,int b0){int a=a0,b=b0; final int GUARD=idom.length*4+8; int guard=0; while(a!=b && guard++<GUARD){int pa=rpoPos[a], pb=rpoPos[b]; if(pa<pb){ b=idom[b]; if(b<0) return a; } else if(pb<pa){ a=idom[a]; if(a<0) return b; } else { a=idom[a]; b=idom[b]; if(a<0||b<0) break; }} return (a<0)? b: a;}
