...
public final class Dominators {
    ...
    public static Dominators compute(ReducedCFG cfg){int[] nodes=cfg.allBlockIds(); Arrays.sort(nodes); int n=nodes.length; if(n==0) return new Dominators(cfg,new int[0],new HashMap<Integer,Integer>(),new int[0],new int[0],new int[0],new int[0]); Map<Integer,Integer> idx=new HashMap<Integer,Integer>(); for(int i=0;i<n;i++) idx.put(nodes[i], i);
        ...
        int[] idom=new int[n]; Arrays.fill(idom,-1); idom[entry]=entry; int[] rpoPos=new int[n]; for(int i=0;i<n;i++) rpoPos[rpo[i]]=i; boolean changed; do{changed=false; for(int i=0;i<n;i++){int b=rpo[i]; if(b==entry) continue; int newI=-1; for(int p:predIdx[b]) if(idom[p]!=-1){newI=p; break;} if(newI==-1) continue; for(int p:predIdx[b]){ if(p==newI||idom[p]==-1) continue; newI=intersect(idom,rpoPos,p,newI);} if(idom[b]!=newI){idom[b]=newI; changed=true;}} }while(changed);
    ...
    private static int[] computeRpo(int entry,int[][] succ,int n){boolean[] seen=new boolean[n]; int[] out=new int[n]; int oi=0; Deque<Integer> stack=new ArrayDeque<Integer>(); Deque<Integer> it=new ArrayDeque<Integer>(); stack.push(entry); it.push(0); while(!stack.isEmpty()){int u=stack.peek(); int k=it.pop(); if(!seen[u]){seen[u]=true; k=0;} if(k<succ[u].length){int v=succ[u][k]; it.push(k+1); if(v>=0 && v<n && !seen[v]){stack.push(v); it.push(0);} } else {out[oi++]=u; stack.pop();}} int[] r=new int[oi]; System.arraycopy(out,0,r,0,oi); for(int i=0;i<n;i++) if(!seen[i]){r=append(r,i);} for(int i=0,j=r.length-1;i<j;i++,j--){int tmp=r[i]; r[i]=r[j]; r[j]=tmp;} return r;}
    private static int[] append(int[] a,int v){int[] r=Arrays.copyOf(a,a.length+1); r[a.length]=v; return r;}
    private static int intersect(int[] idom,int[] rpoPos,int a0,int b0){int a=a0,b=b0; final int GUARD=idom.length*4+8; int guard=0; while(a!=b && guard++<GUARD){int pa=rpoPos[a], pb=rpoPos[b]; if(pa<pb){ int nextB=idom[b]; if(nextB==b || nextB<0) return a; b=nextB; } else if(pb<pa){ int nextA=idom[a]; if(nextA==a || nextA<0) return b; a=nextA; } else { int nextA=idom[a]; int nextB=idom[b]; if(nextA==a || nextB==b) break; if(nextA<0||nextB<0) break; a=nextA; b=nextB; }} return (a<0)? b: a;}
